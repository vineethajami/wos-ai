...
<#  
    The genie_setup.ps1 PowerShell script automatesthe setup process for Qualcomm's AI Engine Direct by downloading and installing necessary components, including Python, ONNX models, QNN SDK, and various dependencies.
    ...
    process{
        ...
        if (Test-Path "$SDX_QAIRT_VENV_Path\Scripts\Activate.ps1") {
            ...
            #update the QNN version in the below command as needed. 
     	    Unblock-File C:\Qualcomm\AIStack\QAIRT\$QNN_SDK_VERSION\bin\envsetup.ps1
        ...
        Check_Setup -logFilePath "$debugFolder\GENIE_Setup_Debug.log"
	Invoke-Command { & "powershell.exe" } -NoNewScope
	Activate_QNN_VENV -rootDirPath $rootDirPath
	"$SDX_QAIRT_VENV_Path\Scripts\Activate.ps1"
...
Function Activate_QNN_VENV {
    param ( 
        ...
        $SDX_QAIRT_VENV_Path = "$rootDirPath\$QAIRT_VENV_Path"
	#echo "printing quari path"
	#echo $SDX_QAIRT_VENV_Path
        $global:DIR_PATH      = $rootDirPath
        & "$SDX_QAIRT_VENV_Path\Scripts\Activate.ps1"
        & C:\Qualcomm\AIStack\QAIRT\$QNN_SDK_VERSION\bin\envsetup.ps1
	copy $QNN_SDK_ROOT\lib\x86_64-windows-msvc\Genie.dll $QNN_SDK_ROOT\lib\python\qti\aisw\genai\windows-x86_64
 	copy $QNN_SDK_ROOT\lib\x86_64-windows-msvc\QnnGenAiTransformerComposerQuantizer.dll $QNN_SDK_ROOT\bin\aarch64-windows-msvc
